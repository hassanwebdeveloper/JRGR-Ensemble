...
class TransGANGenerator(nn.Module):
    """docstring for TransGANGenerator"""
    def __init__(self, depth1=5, depth2=5, depth3=4, depth4=4, depth5=2, depth6=2, initial_size=8, dim=384, heads=4, mlp_ratio=4, drop_rate=0.):#,device=device):
        super(TransGANGenerator, self).__init__()
        ...
        self.positional_embedding_4 = nn.Parameter(torch.zeros(1, (8 * 8) ** 2, 384 // 32))
        self.positional_embedding_5 = nn.Parameter(torch.zeros(1, (8 * 16) ** 2, 384 // 64))
        self.positional_embedding_6 = nn.Parameter(torch.zeros(1, (8 * 32) ** 2, 384 // 128))
        ...
        self.TransformerEncoder_encoder4 = TransformerEncoder(depth=self.depth4, dim=self.dim//32, heads=self.heads, mlp_ratio=self.mlp_ratio, drop_rate=self.droprate_rate)
        self.TransformerEncoder_encoder5 = TransformerEncoder(depth=self.depth5, dim=self.dim // 64, heads=self.heads, mlp_ratio=self.mlp_ratio, drop_rate=self.droprate_rate)
        self.TransformerEncoder_encoder6 = TransformerEncoder(depth=self.depth6, dim=self.dim // 128, heads=self.heads, mlp_ratio=self.mlp_ratio, drop_rate=self.droprate_rate)
    ...
    def forward(self, noise):
        ...
        x = x + self.positional_embedding_6
        x = self.TransformerEncoder_encoder6(x)

        x = self.linear(x.permute(0, 2, 1).view(-1, self.dim//128, H, W))
